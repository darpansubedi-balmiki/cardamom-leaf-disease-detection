...
from .models.classifier import (
    ...
    DEFAULT_TOP_K,
    DiseaseClassifier,
    PredictionResult,  # If this does not exist in your project, remove this import + type hint below.
)
...
def _prediction_to_response(
    result: PredictionResult,  # if PredictionResult import fails, change this to: result
    threshold: float,
...
def _find_target_conv2d(model: torch.nn.Module) -> Optional[torch.nn.Conv2d]:
    ...
    Tries EfficientNet-style `model.features[-1]` first, then falls back to last Conv2d.
    """
    # Prefer the last feature block's conv (EfficientNet-like)
    try:
